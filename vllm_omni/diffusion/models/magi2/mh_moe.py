...
class Magi2MultiHeadMoE(nn.Module):
    ...
    def forward(self, x: torch.Tensor, sequence_split_sizes: list[int] | None = None) -> torch.Tensor:
        ...
        if sequence_split_sizes is not None and (
            len(sequence_split_sizes) != self.ep_group.world_size
            or any(type(size) is not int or size < 0 for size in sequence_split_sizes)
            or sequence_split_sizes[self.ep_group.rank] != x_heads.shape[0]
        ):
            raise ValueError("EP token counts must describe the local tensor in EP group rank order")
