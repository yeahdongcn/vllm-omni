...
class Magi2GroupedLinear(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if bias:
            self.bias = nn.Parameter(torch.empty(num_experts * out_features, dtype=dtype, device=device))
        else:
    ...
    def forward(
        ...
        bias = self.bias.view(self.num_experts, self.out_features) if self.bias is not None else None
        if self.num_experts == 1:
            return F.linear(tensor, weight[0], None if bias is None else bias[0])
        ...
        outputs = [
            F.linear(part, weight[index], None if bias is None else bias[index]) for index, part in enumerate(inputs)
        ]
