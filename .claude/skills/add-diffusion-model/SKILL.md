...
For models without a diffusers pipeline — weights in custom format, model code in a private repo. Real examples: BAGEL, HunyuanImage3. See [references/custom-model-patterns.md](references/custom-model-patterns.md) for concrete custom-model integration patterns.
...
| **External dependency code** (original repo utilities) | **External repo**, installed via download script or pip | original-repo package via git clone |
